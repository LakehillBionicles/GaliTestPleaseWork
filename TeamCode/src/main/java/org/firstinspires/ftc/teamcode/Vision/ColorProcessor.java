...
public class ColorProcessor extends OpenCvPipeline {
    ...
    public Mat processFrame(Mat input) {
        ...
        for (int i = 1; i < widthOfInput - anchorWidth; i++) {
            Point BarPoint1 = new Point(i, 150);
            Point BarPoint2 = new Point(i + anchorWidth, heightOfInput);
        ...
        if(positionOfBlueObject<(widthOfInput/3)){
            positionOfBlueObject = 1;
        }
        if(positionOfBlueObject<((2*widthOfInput)/3)&&positionOfBlueObject>(widthOfInput/3)){
            positionOfBlueObject = 2;
        }
        if(positionOfBlueObject>((2*widthOfInput)/3)){
            positionOfBlueObject = 3;
        }
        if(positionOfRedObject<(widthOfInput/3)){
            positionOfRedObject = 1;
        }
        if(positionOfRedObject<((2*widthOfInput)/3)&&positionOfRedObject>(widthOfInput/3)){
            positionOfRedObject = 2;
        }
        if(positionOfRedObject>((2*widthOfInput)/3)){
            positionOfRedObject = 3;
        }
        return input;
    }
    public static int getPositionOfBlueObject(){return positionOfBlueObject;}
    public static int getPositionOfRedObject(){return positionOfRedObject;}
